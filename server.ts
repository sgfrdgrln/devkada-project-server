...
async function getWorker() {
	if (!worker) {
		...
		await worker.setParameters({
			tessedit_pageseg_mode: PSM.SINGLE_BLOCK, // block of text (best for receipts)
...
const idRefRegex =
	/\b(id|ref|reference|order|transaction|trans|trx|tnx|rrn|stan|approval|auth|invoice|inv|terminal|serial|trace)\b/i;
const accountRegex =
	/\b(customer account|account number|acct number|can)\b/i;

const phoneRegex = /\+?\d[\d\s-]{9,}/;

const currencyRegex =
	/(\$|\u20B1|\bphp\b|\bpeso(?:s)?\b|\bP\s*\d)/i;

const keywordWeights = [
	{ pattern: /\bplease pay\b/i, score: 140 },
	{ pattern: /\btotal amount sent\b/i, score: 130 },
    { pattern: /\bamount\b/i, score: 130 },
	{ pattern: /\bgrand total\b/i, score: 120 },
	{ pattern: /\btotal amount\b/i, score: 120 },
    { pattern: /\btotal amount due\b/i, score: 120 },
...
const hasKeyword = (text: string) => getKeywordScore(text) > 0;

const hasCurrency = (text: string) =>
	currencyRegex.test(text);

const hasDateTime = (line: string) =>
...
const scoreCandidates = (raw: string): Candidate[] => {
	...
	for (let i = 0; i < lines.length; i++) {
		...
		for (const match of matches) {
			...
			if (lineHasDateTime || lineHasIdRef || lineHasAccount) score -= 100;
